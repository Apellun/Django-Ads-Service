...
В API используется кастомная модель пользователя и библиотека Djoser для CRUD пользователей, авторизации и аутентификации. API работает с базой данных SQLite в режиме разработки и с PostgreSQL в режиме продакшена. К API подключена документация Swagger.</p>
